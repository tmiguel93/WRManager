    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        <Card className="premium-card">
          <CardHeader>
            <CardTitle className="font-heading text-xl">{t("practice.weekendTitle", "Weekend Context")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {view.event ? (
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
                  {t("practice.round", "Round")} {view.event.round} · {trackTypeLabel(view.event.trackType)}
                </p>
        ...
        <Card className="premium-card">
          <CardHeader>
            <CardTitle className="font-heading text-xl">{t("practice.setupTitle", "Setup Board")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">{t("practice.runPlan", "Run Plan")}</p>
              <select
                value={runPlan}
              ...
              >
                <option value="SHORT">{t("practice.shortRuns", "Short Runs")}</option>
                <option value="BALANCED">{t("practice.balancedProgram", "Balanced Program")}</option>
                <option value="LONG">{t("practice.longRuns", "Long Runs")}</option>
              </select>
            </div>

            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
                {t("practice.aeroBalance", "Aero Balance")} ({aeroBalanceFocus})
              </p>
              <input
                type="range"
                min={-100}
              ...
              />
              <p className="text-xs text-muted-foreground">{t("practice.aeroHint", "-100 = high-downforce · +100 = low-drag")}</p>
            </div>

            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
                {t("practice.weatherBias", "Dry/Wet Bias")} ({weatherFocus})
              </p>
              <input
                type="range"
                min={-100}
              ...
              />
              <p className="text-xs text-muted-foreground">{t("practice.weatherHint", "-100 = dry focus · +100 = wet-safe setup")}</p>
            </div>
            ...
            {firstPendingSession ? (
              ...
              >
                {t("practice.runNext", "Run Next Practice Session")} ({firstPendingSession.tokenLabel})
              </Button>
            ) : (
              <div className="rounded-2xl border border-emerald-300/25 bg-emerald-500/10 p-3 text-xs text-emerald-100">
                {t("practice.allDone", "All practice sessions completed for this weekend.")}
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
        <Card className="premium-card">
          <CardHeader>
            <CardTitle className="font-heading text-xl">{t("practice.sessionsTitle", "Practice Sessions")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {view.practiceSessions.map((session) => (
              <div key={session.id} className="rounded-2xl border border-white/10 bg-white/5 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  >
                    {session.completed ? t("practice.completed", "Completed") : t("practice.pending", "Pending")}
                  </Badge>
                </div>
                ...
                {session.completed ? (
                  <div className="mt-2 grid gap-2 md:grid-cols-3">
                    <div className="rounded-xl border border-white/10 bg-white/5 p-2 text-xs">
                      {t("practice.setup", "Setup")}: {session.setupConfidence ?? "-"}
                    </div>
                    <div className="rounded-xl border border-white/10 bg-white/5 p-2 text-xs">
                      {t("practice.track", "Track")}: {session.trackKnowledge ?? "-"}
                    </div>
                    <div className="rounded-xl border border-white/10 bg-white/5 p-2 text-xs">
                      {t("practice.pace", "Pace")}: {session.paceDelta && session.paceDelta >= 0 ? "+" : ""}
                      {session.paceDelta ?? "-"}
                    </div>
                  </div>
                ) : (
                  <Button
                    ...
                    disabled={isPending || !view.event?.hasWeekend}
                  >
                    {t("practice.runThis", "Run This Session")}
                  </Button>
                )}
              </div>
            ))}
            ...
            {view.practiceSessions.length === 0 ? (
              <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-muted-foreground">
                {t("practice.noSessions", "No practice sessions available yet. Generate the weekend first.")}
              </div>
            ) : null}
          </CardContent>
        </Card>
        ...
        <Card className="premium-card">
          <CardHeader>
            <CardTitle className="font-heading text-xl">{t("practice.feedbackTitle", "Driver Feedback Board")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
              ...
              <div key={driver.id} className="rounded-2xl border border-white/10 bg-white/5 p-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge className="rounded-full border border-white/15 bg-white/10 text-xs">
                    {t("practice.ovr", "OVR")} {driver.overall}
                  </Badge>
                </div>
              </div>
            ))}
