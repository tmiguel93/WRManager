...
export async function getRaceControlCenterView(
  ...
  const startingGrid: RaceGridRow[] = gridOrder
    ...
    .map((driver) => ({
      ...
      teamLogoUrl: driver.currentTeam?.logoUrl ?? null,
      teamPrimaryColor: driver.currentTeam?.primaryColor ?? "#0ea5e9",
      teamSecondaryColor: driver.currentTeam?.secondaryColor ?? "#facc15",
      teamAccentColor: driver.currentTeam?.accentColor ?? null,
  ...
  const leaderboard: RaceLeaderboardRow[] = raceRows.map((row) => ({
    ...
    teamLogoUrl: row.driver.currentTeam?.logoUrl ?? null,
    teamPrimaryColor: row.driver.currentTeam?.primaryColor ?? "#0ea5e9",
    teamSecondaryColor: row.driver.currentTeam?.secondaryColor ?? "#facc15",
    teamAccentColor: row.driver.currentTeam?.accentColor ?? null,
